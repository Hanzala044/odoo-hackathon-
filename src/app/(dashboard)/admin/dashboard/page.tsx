...
export default async function AdminDashboardPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const pendingLeaves = await prisma.leaveRequest.count({ where: { status: "PENDING", user: { companyId: session.companyId } } });
  const filtered = q
    ? users.filter((u) => `${u.profile?.firstName} ${u.profile?.lastName} ${u.email}`.toLowerCase().includes(q.toLowerCase()))
    : users;
  const presentToday = users.filter((u) => (u as any).attendances[0]?.checkIn && !(u as any).attendances[0]?.checkOut).length;
  const onLeaveToday = users.filter((u) => (u as any).leaveRequests.length > 0).length;
    ...
    <>
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">{company?.name} · Manage your organization in real time</p>
        </div>
        {company && <CompanyLogoForm companyName={company.name} companyLogo={company.logo} />}
      </div>
            ...
            >
              ...
              <div className="flex items-center gap-4">
                {u.profile?.profilePic ? (
                  ...
                  <img src={u.profile.profilePic} alt="" className="h-12 w-12 rounded-full object-cover" />
                ) : (
                ...
                <div className="min-w-0">
                  ...
                  <p className="truncate text-[11px] text-muted">{u.employeeId} · {u.role}</p>
                </div>
              </div>
            </Link>
          );
